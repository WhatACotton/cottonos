...
// FRESULT read_data_to_entry_point(const char *filename, void **entry_point, size_t buffer_size)
// {
...
//     {
//         size_t to_read = (BUFFER_SIZE - total_read < BUFFER_SIZE) ? (BUFFER_SIZE - total_read) : BUFFER_SIZE;
...
int main()
{
    ...
    static char buf[16];
    size_t data_size = sizeof(os_bin);
    char *entry_point;
    void (*f)(void);
    ...
    // バイナリデータを書き込む
    res = write_binary_to_sdcard(filename, os_bin, data_size);
    if (res == FR_OK)
    {
...
// int main()
// {
//     const char *filename = "example.bin"; // 読み込むELFファイル名
//     char *entry_point;
//     int data_size = sizeof(os_bin);
//     void (*f)(void);

//     init_platform();
//     FRESULT res;

//     xil_printf("Loading ELF from %s\n\r", filename);

//     // ELFファイルを読み込む
//     res = read_data_to_entry_point(filename, (void **)&entry_point, data_size);
//     if (res != FR_OK)
//     {
//         xil_printf("Error loading ELF: %d\n", res);
//         return -1;
//     }

//     // 読み込んだデータをダンプ
//     xil_printf("Loaded ELF data:\n");
//     dump(entry_point, BUFFER_SIZE);
//     // ELFファイルのエントリーポイントを実行
//     co_puts("Starting execution from entry point...\n");
//     // f = (void (*)(void))entry_point; // エントリーポイントを設定
//     // f();                             // ELFファイルを実行
