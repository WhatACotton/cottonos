...
/* ELF�إå��Υ����å� */
static int co_elf_check(struct elf_header *header)
{
  xil_printf("co_elf_check\r\n");
  // if (co_memcmp(header->id.magic, "\x7f"
  //                                 "ELF",
  //               4))
  //   return -1;
...
/* セグメント単位でのロード */
static int co_elf_load_program(struct elf_header *header)
{
  xil_printf("co_elf_load_program\r\n");
  int i;
